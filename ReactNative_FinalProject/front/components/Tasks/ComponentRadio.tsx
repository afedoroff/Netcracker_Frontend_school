...
type NewTaskRadioProps = {
    value: string,
    optionsArray: { label: string; value: string; color: string, icon: string }[],
    onChange: (...event: any[]) => void
}

const ComponentRadio = ({value, optionsArray, onChange}: NewTaskRadioProps) => {
    ...
    return (
        <Radio.Group defaultValue={value} value={value} size="lg" name="componentGroup" accessibilityLabel="pick component"
                     onChange={onChange}
        >
            {
                optionsArray.map(option => {
                    return (
                        ...
                        >
                            ...
                            <Text style={styles.component_text}>{option.label}</Text>
                        </Radio>
